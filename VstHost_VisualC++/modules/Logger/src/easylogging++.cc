...
void Configurations::setToDefault(void) {
  ...
  set(Level::Debug, ConfigurationType::Format,
      std::string("[%datetime] [%level] [%user@%host] [%fbase::%line] %msg"));
  // INFO and WARNING are set to default by Level::Global
  set(Level::Error, ConfigurationType::Format, std::string("[%datetime] [%level] [%func] %msg"));
  set(Level::Fatal, ConfigurationType::Format, std::string("[%datetime] [%level] [%func] %msg"));
...
base::type::string_t DateTime::formatTime(unsigned long long time, base::TimestampUnit timestampUnit) {
  ...
  for (base::type::EnumType i = start; i < base::consts::kTimeFormatsCount - 1; ++i) {
    ...
    if (base::consts::kTimeFormats[i].value == 1000.0f && static_cast<float>(time) / 1000.0f < 1.9f) {
      break;
    }
  ...
  for (; *format; ++format) {
    if (*format == base::consts::kFormatSpecifierChar) {
      ...
      case '\0':  // End
        ...
        break;
      case 'd':  // Day
        buf = base::utils::Str::convertAndAddToBuff(static_cast<unsigned long>(tInfo->tm_mday), 2, buf, bufLim);
        continue;
      ...
      case 'A':  // Day of week (long)
        ...
        continue;
      case 'M':  // month
        buf = base::utils::Str::convertAndAddToBuff(static_cast<unsigned long>(tInfo->tm_mon + 1), 2, buf, bufLim);
        continue;
      case 'b':  // month (short)
      ...
      case 'y':  // year (two digits)
        buf = base::utils::Str::convertAndAddToBuff(static_cast<unsigned long>(tInfo->tm_year + base::consts::kYearBase), 2, buf, bufLim);
        continue;
      case 'Y':  // year (four digits)
        buf = base::utils::Str::convertAndAddToBuff(static_cast<unsigned long>(tInfo->tm_year + base::consts::kYearBase), 4, buf, bufLim);
        continue;
      case 'h':  // hour (12-hour)
        buf = base::utils::Str::convertAndAddToBuff(static_cast<unsigned long>(tInfo->tm_hour % 12), 2, buf, bufLim);
        continue;
      case 'H':  // hour (24-hour)
        buf = base::utils::Str::convertAndAddToBuff(static_cast<unsigned long>(tInfo->tm_hour), 2, buf, bufLim);
        continue;
      case 'm':  // minute
        buf = base::utils::Str::convertAndAddToBuff(static_cast<unsigned long>(tInfo->tm_min), 2, buf, bufLim);
        continue;
      case 's':  // second
        buf = base::utils::Str::convertAndAddToBuff(static_cast<unsigned long>(tInfo->tm_sec), 2, buf, bufLim);
        continue;
...
void LogFormat::updateDateFormat(std::size_t index, base::type::string_t& currFormat) {
  ...
  if ((currFormat.size() > index) && (ptr[0] == '{')) {
    ...
    currFormat.erase(index, static_cast<unsigned long>(count));
  ...
  if ((dateIndex = resultingFilename.find(dateTimeFormatSpecifierStr.c_str())) != std::string::npos) {
    ...
    if (dateIndex != std::string::npos) {
      ...
      if ((resultingFilename.size() > dateIndex) && (ptr[0] == '{')) {
        ...
        resultingFilename.erase(dateIndex + dateTimeFormatSpecifierStr.size(), static_cast<long unsigned int>(count));
        fmt = ss.str();
      } else {
...
void DefaultLogDispatchCallback::dispatch(base::type::string_t&& logLine) {
  if (m_data->dispatchAction() == base::DispatchAction::NormalLog) {
    if (m_data->logMessage()->logger()->m_typedConfigurations->toFile(m_data->logMessage()->level())) {
      ...
      if (fs != nullptr) {
        fs->write(logLine.c_str(), static_cast<long int>(logLine.size()));
        if (fs->fail()) {
...
Writer& Writer::construct(int count, const char* loggerIds, ...) {
  if (ELPP->hasFlag(LoggingFlag::MultiLoggerSupport)) {
    ...
    m_loggerIds.reserve(static_cast<long unsigned int>(count));
