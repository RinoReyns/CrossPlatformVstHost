...
AUDIOHOSTLIB_EXPORT int AudioProcessingVstHost::ApplyFilter(const std::string& input_wave_path, const std::string& output_wave_path)

{
	...
	BWLowPass* filter = create_bw_low_pass_filter(
		8,
		static_cast<float>(input_wave_file.sample_rate()),
		500);
	if (filter == NULL)
	{
	...
	for (int i = 0; i < content.size(); i++) {
		output[i] = bw_low_pass(filter, content[i]);
	}
