...
class File::Impl {
 public:
  Error WriteHeader(uint64_t data_size) {
    ...
    header.riff.chunk_size = static_cast<uint32_t>(sizeof(WAVEHeader) + (data_size * (bits_per_sample / 8)) - 8);
    // fmt header
  ...
  Error ReadHeader(HeaderList* headers) {
    ...
    data_offset_ = data_header.position() + sizeof(data_header.chunk_size()) + (data_header.chunk_id().size() 
                    * sizeof(char));
...
Error File::Read(uint64_t frame_number, void(*decrypt)(char* data, size_t size), std::vector<float>* output)
{
  ...
  for (size_t sample_idx = 0; sample_idx < output->size(); sample_idx++) {
    if (impl_->header.fmt.bits_per_sample == 8) {
      ...
      impl_->istream.read(reinterpret_cast<char*>(&value), sizeof(value));
      decrypt(reinterpret_cast<char*>(&value), sizeof(value) / sizeof(char));
      (*output)[sample_idx] = static_cast<float>(value) / static_cast<float>(std::numeric_limits<int32_t>::max());
    } else {
      return kInvalidFormat;
    }
  }
...
Error File::Write(const std::vector<float>& data,
  ...
  for (auto sample : data) {
    ...
    if (bits_per_sample == 8) {
      ...
      // 24bits int doesn't exist in c++. We create a 3 * 8bits struct to
      // simulate
      int v = static_cast<int>(sample * INT24_MAX);
      int8_t value[3];
      ...
      // 32bits
      int32_t value = static_cast<int32_t>(sample * static_cast<float>(std::numeric_limits<int32_t>::max()));
      encrypt(reinterpret_cast<char*>(&value), sizeof(value) / sizeof(char));
      impl_->ostream.write(reinterpret_cast<char*>(&value), sizeof(value));
    } else {
      return kInvalidFormat;
